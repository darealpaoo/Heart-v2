...
Object.assign(Easing, {
  InQuad: (t) => t * t,
  OutQuad: (t) => 1 - (1 - t) * (1 - t),
  InOutQuad: (t) => (t < 0.5 ? 2 * t * t : 1 - x(-2 * t + 2, 2) / 2),
  InCubic: (t) => t * t * t,
  OutCubic: (t) => 1 - x(1 - t, 3),
  InOutCubic: (t) => (t < 0.5 ? 4 * t * t * t : 1 - x(-2 * t + 2, 3) / 2),
  InQuart: (t) => t * t * t * t,
  OutQuart: (t) => 1 - x(1 - t, 4),
  InOutQuart: (t) => (t < 0.5 ? 8 * t * t * t * t : 1 - x(-2 * t + 2, 4) / 2),
  InQuint: (t) => t * t * t * t * t,
  OutQuint: (t) => 1 - x(1 - t, 5),
  InOutQuint: (t) =>
    t < 0.5 ? 16 * t * t * t * t * t : 1 - x(-2 * t + 2, 5) / 2,
  InSine: (t) => 1 - q((t * B) / 2),
  OutSine: (t) => T((t * B) / 2),
  ...
  InOutCirc: (t) =>
    t < 0.5 ? (1 - C(1 - x(2 * t, 2))) / 2 : (C(1 - x(-2 * t + 2, 2)) + 1) / 2,
  InBack: (t) => 2.70158 * t * t * t - F * t * t,
  OutBack: (t) => 1 + 2.70158 * x(t - 1, 3) + F * x(t - 1, 2),
  InOutBack: (t) =>
    t < 0.5
      ? (x(2 * t, 2) * (2 * (M + 1) * t - M)) / 2
      : (x(2 * t - 2, 2) * ((M + 1) * (2 * t - 2) + M) + 2) / 2,
  InElastic: (t) =>
    0 === t ? 0 : 1 === t ? 1 : -x(2, 10 * t - 10) * T((10 * t - 10.75) * Q),
  OutElastic: (t) =>
    0 === t ? 0 : 1 === t ? 1 : x(2, -10 * t) * T((10 * t - 0.75) * Q) + 1,
  InOutElastic: (t) =>
    0 === t
      ? 0
      : 1 === t
      ? 1
      : t < 0.5
      ? (-x(2, 20 * t - 10) * T((20 * t - 11.125) * j)) / 2
      : (x(2, -20 * t + 10) * T((20 * t - 11.125) * j)) / 2 + 1,
  InBounce: (t) => 1 - N(1 - t),
  OutBounce: N,
  InOutBounce: (t) =>
    t < 0.5 ? (1 - N(1 - 2 * t)) / 2 : (1 + N(2 * t - 1)) / 2,
});
...
class FlowInstance {
  ...
  update(now = performance.now()) {
    ...
    if (typeof this.waitCondition == "number") {
      if (now < this.waitCondition) return 0;
    } else if (typeof this.waitCondition == "function")
      if (!this.waitCondition()) return this.waitCondition;

    this.waitCondition = this.flow.next().value;
    if (typeof this.waitCondition == "number") this.waitCondition += now;
    if (this.waitCondition === undefined) this.thenCb && this.thenCb();
...
export class Flow {
  ...
  tweenVector3 = function ({
    ...
    this.start(
      function* ({ object, value, start, end, delay, duration, easing }) {
        ...
        let vStart =
          start ||
          new Vector3().copy(
            Vector3.prototype.set.call(object[value], 0.01, 0.01, 0.01)
          );
        yield delay;
        ...
        while (tNow < tStart + duration) {
          ...
          Vector3.prototype.lerpVectors.call(
            object[value],
            vStart,
            vEnd,
            easing ? easing(alpha) : alpha
          );
          yield 0;
